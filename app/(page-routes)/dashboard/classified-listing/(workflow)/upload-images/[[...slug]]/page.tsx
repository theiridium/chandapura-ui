...
const Page = ({ params }: { params: { slug: string } }) => {
    ...
    const [imageParamsFeatured, setImageParamsFeatured] = useState<ImageParams>({
        refId: source,
        ref: "api::classified-listing.classified-listing",
        ...
        try {
            ...
            if (type === "edit") {
                ...
                if (response.data) {
                    ...
                    router.push(`/dashboard/classified-listing/publish?type=${type}&source=${response.data.id}`)
                }
            }
        } catch (error) {
            ...
            <div className='col-span-full lg:col-span-6 mt-3 lg:my-8'>
                ...
                <div className='grid grid-cols-1 gap-10 mx-2'>
                    ...
                    <div className='flex gap-x-5 justify-end text-xl *:w-auto *:rounded-lg *:mb-5 *:py-2 *:px-5 *:block font-semibold'>
                        <Button className='btn-primary text-base' color='primary' isDisabled={isSubmitLoading} onClick={() => router.push(`/dashboard/classified-listing/add-details?type=edit_back&source=${source}`)}>
                            Back
                        </Button>
