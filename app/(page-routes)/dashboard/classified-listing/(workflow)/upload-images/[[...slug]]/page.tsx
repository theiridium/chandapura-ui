...
const Page = ({ params }: { params: { slug: string } }) => {
    ...
    const onClickSave = async () => {
        try {
            ...
            if (type === "edit") {
                ...
                router.push(`/dashboard/classified-listing/publish?type=${type}&source=${source}`)
            }
        } catch (error) {
    ...
    const setFormBtnEl = () => (
        <div key={1} className='flex gap-x-5 justify-end text-xl *:w-auto *:rounded-lg p-2 *:py-2 *:px-5 *:block font-semibold'>
            <Button className='btn-primary text-base' color='primary' isDisabled={isSubmitLoading || isloading} onPress={() => router.push(`/dashboard/classified-listing/add-details?type=edit_back&source=${source}`)}>
                Back
            </Button>
