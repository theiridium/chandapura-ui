        ...
        <>
            {!!job_details && <a href={`${Products.job.url}/${item.category}/${item.slug}?source=${id}`} className="re-card-link">
                <div className="job-card p-5">
                    ...
                    <div className="text-sm text-slate-600 mb-5 flex items-center"><Banknote size={15} className='mr-2' /><IndianRupee size={15} /> {ConvertCurrencyToWords(job_details.salary_range_min)} - <IndianRupee size={15} /> {ConvertCurrencyToWords(job_details.salary_range_max)} <span className='ml-1'>{job_details?.salary_type.toLowerCase()}</span></div>
