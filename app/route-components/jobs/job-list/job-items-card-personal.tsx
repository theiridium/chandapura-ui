        ...
        <a className="card_link" href={`/${product.slug}/${data.category}/${data.slug}?source=${id}`}>
            <div className="rounded-xl bg-white">
                <div className="job-card p-5 w-auto">
                    <div className='flex flex-row gap-5 items-center'>
                        <div className="*:h-16 *:aspect-square">
                            {data.logo_image === null ?
                                <img src="/images/icons/personal-logo-placeholder.jpg" /> :
                    ...
                    <div className="text-sm text-slate-600 flex items-center"><Banknote size={15} className='mr-2' /><IndianRupee size={15} /> {ConvertCurrencyToWords(job_details.salary_range_min)} - <IndianRupee size={15} /> {ConvertCurrencyToWords(job_details.salary_range_max)} yearly</div>
